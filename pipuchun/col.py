...
async def bd(message: types.Message):
    ...
    if message.chat.type == 'private':
        if check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=message.from_user.id)) & check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID2, user_id=message.from_user.id)):
                          ...
                          await bot.send_message(message.from_user.id, '📝 To\'g\'ri jumlalarni yozing : ', reply_markup=nav.sainmenu )  
                          btext = message.text
                          try:
                                
                                 respond = wikipedia.summary(btext)
                                 
                                
                                 await bot.send_message(message.from_user.id, f'{respond}', reply_markup=nav.sainmenu )  
...
async def botmeseg(message: types.Message):
    ...
    if message.chat.type == 'private':
        if check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=message.from_user.id)) & check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID2, user_id=message.from_user.id)):
           ...
           if message.text == 'Quron 🎧':
              ...
              await bot.send_message(message.from_user.id, f'  <b>Yuqoridagi tugmadan qayta foydalanishingiz mumkin ⤴️</b>', reply_markup=nav.sainmenu)
               ...
               await bot.send_document(message.from_user.id, document="https://ia801701.us.archive.org/28/items/dli.pahar.1232/1891%20Report%20of%20a%20Journey%20in%20Chinese%20Turkistan%20in%201889-90%20by%20Bower%20%28secret%20and%20confidential%29%20missing%20map%20s.pdf")
        ...
        #                             await bot.send_message(message.from_user.id, f'{i}', reply_markup=nav.sainmenu )  
